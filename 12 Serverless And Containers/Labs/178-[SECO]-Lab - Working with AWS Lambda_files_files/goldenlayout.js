...
lm.utils.copy( lm.controls.BrowserPopout.prototype, {
	...
	popIn: function() {
		...
		/*
		 * After the child item is re-added into the parent, remove the child from list of
		 * opened popouts (from parent Golden Layout Instnace)
		 */
		if(parentItem.layoutManager.openPopouts.length > 0) {
			var popoutIndex = parentItem.layoutManager.openPopouts.indexOf(this);
	  	if(popoutIndex > -1) parentItem.layoutManager.openPopouts.splice(popoutIndex,  1);
	  	
			// When closing a popout, ensure all gl-window entries stored in localStorage are removed
			for (let i = 0; i < localStorage.length; i++) {
				if((localStorage.key(i)).indexOf('gl-window-config') > -1) {
	...
	_createWindow: function() {
		...
		console.log("====== Popout Window Data ======");
		console.log(url);
		new_url = url.replace('https://rizzian.vocareum.com', "..");
		url = new_url;
		console.log(new_url);
...
lm.utils.copy( lm.controls.DragProxy.prototype, {
	...
	_setDropPosition: function( x, y ) {
		...
		if( this._area !== null ) {
			...
			// VOC CHANGES			
			// if area being dragged is root, check to see if area root has invalidated drop zones
			if(this._area.side && this._area.contentItem.config.invalidDropZones 
			&& this._area.contentItem.config.invalidDropZones.includes(this._area.side)) {
				this._area = null; return;
			}
	...
	_onDrop: function() {
		...
		if( this._area !== null ) {
			...
			if(!$(this._originalParent.element).is(':visible')) {
				
				// gets all stacks in the layout
				var stacks = this._layoutManager.root.getItemsByType('stack');
				
				if (stacks.length > 0) {
					var newTargetStackIndex = null;
					
					// itterates through stack
					for (var i = 0; i < stacks.length; i++) {
						if (stacks[i].config.hasHeaders) {
							newTargetStackIndex = i;
							break;
						}
					}
					if (newTargetStackIndex !== null) {
						// add content item to last stack 
						stacks[newTargetStackIndex].addChild(this._contentItem);
					} else {
...
lm.utils.copy( lm.controls.Tab.prototype, {
	...
	_onTabClick: function( event ) {
		...
		if( !$(event.target).hasClass('lm_close_tab') && (event.button === 0 || event.type === 'touchstart') ) {
...
lm.utils.copy( lm.items.AbstractContentItem.prototype, {
	...
	_$highlightDropZone: function( x, y, area ) {
		...
		// VOC CHANGES
		// if side is x1, it's right side
		// if area is x2, it's left side
		// if area is y1, bottom
		// if side is y2, top
		//if(this.config.invalidDropZones && this.config.invalidDropZones.includes(area.side)) return null;
		
		// if invalid dropzone, root cannot have zone created
		//if(this.config.invalidDropZone) return null; 
...
lm.utils.copy( lm.items.Stack.prototype, {
		...
		for( segment in this._contentAreaDimensions ) {
			...
			if( area.x1 < x && area.x2 > x && area.y1 < y && area.y2 > y ) {
				...
				if( segment === 'header' ) {
					...
					// If config has invalidDropZone, only allow for header placement
					
					//console.log(this);
					//console.log("Segment: " + segment);
					
					//if(this.config.invalidDropZone) return null;
					if(this.config.invalidDropZones && this.config.invalidDropZones.includes(segment)) return null;
	...
	_$getArea: function() {
		...
		// if it has invalid, return null
		if(this._activeContentItem && this._activeContentItem.config.invalidDropZone) {
			return null;
		}
