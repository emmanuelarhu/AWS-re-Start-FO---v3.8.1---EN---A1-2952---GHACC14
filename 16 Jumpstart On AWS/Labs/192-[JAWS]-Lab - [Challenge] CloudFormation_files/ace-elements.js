...
(function($ , undefined) {
    $.fn.make_accesible = function() {
        $(this).attr('role', 'button'); // Tell assistive technology it's a button
        $(this).attr('tabindex', '0');  // Make it keyboard focusable using tab
    };
})(window.jQuery);

(function($ , undefined) {
    $.fn.attach_accesible_handler = function() {
        $(this).keydown(  function(e) { // Trigger the click event from the keyboard
            var code = e.which;
			//if (code === 9) {
				//disable hovering if possible
				//have to enable hovering on mouse movement
            //}
            // 13 = Return, 32 = Space
            if ((code === 13) || (code === 32)) {
                e.preventDefault();
